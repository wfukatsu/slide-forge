...
| ステージ別の体験と感情の推移 | `journey_map` | 感情値は観察された事実に紐づける |
...
d.testimonial(x, y, w, h, quote, name,
              ...
              portrait=None,  # ピクトグラムの代わりに置く AI 生成画像
              quote_size=13)
```

- 左に人物ピクトグラムと氏名・肩書、右に引用文（“ ” 付き・primary 濃色）。
- `portrait` に描く内容（例「30代の会社員の女性」）を渡すと、ピクトグラムの代わりに
  生成画像をひと回り大きく置く。Gemini の画像生成が ON である必要がある。結果は
  プロンプト単位でキャッシュされるので、作り直しても同じ顔が出る。
  **架空のペルソナにだけ使うこと。実在の人物の発言に生成した顔を付けてはならない**
  （その人の似顔を捏造することになる）。
...
## journey_map — カスタマージャーニーマップ

```python
d.journey_map(x, y, w, h, stages, rows, emotions,
              # stages:   横軸のステージ名（2 つ以上）
              # rows:     [(レイヤー名, [ステージごとのセル]), …]
              #           セル内の改行がそのまま箇条書きの行になる
              # emotions: ステージと同数の -2〜+2 の整数
              emotion_index=None,  # 感情曲線を入れる行位置（既定は最終行の直前）
              label_w=1.2,         # 左のレイヤー名列の幅
              curve_h=0.86,        # 感情曲線の行の高さ
              corner=None,         # 左上のセル（既定は cjm.stage）
              emotion_label=None)  # 感情曲線の行名（既定は cjm.emotion）
```

- 横軸がステージ、縦軸がレイヤーの格子に、中立の破線を挟んだ感情曲線を重ねる。
- 曲線は `linechart` ではなくこの関数が描く。各点をステージ列の中心に合わせる
  必要があり、ステージ数はデータ由来なのでテンプレート側では列の中心座標を
  計算できないため。`linechart` は自前の軸領域と凡例も持ち込んでしまう。
- 表情は円・点・直線で組む。Slides には開き角を指定できる弧がないため
  （`rating_matrix` のハーベイボールと同じ制約）。白黒印刷でも判別できる。
- **感情値は観察された事実（発言・行動・数値）に紐づけること。** 物語として
  見栄えするように上下させた曲線は、発見ではなく作図にすぎない。

```json
{ "type": "journey_map", "x": 0.5, "y": 1.55, "w": 9.0, "h": 3.15,
  "stages": ["認知・興味", "比較・検討", "購入する", "利用する"],
  "rows": [["行動", ["・SNSで知る", "・比較サイトで絞る", "・注文を確定する", "・受け取る"]],
           ["思考・感情", ["・高いかも", "・在庫が不安", "・欠品で落胆", "・手間が残る"]],
           ["課題・要求", ["・価格帯を先に", "・在庫を一覧で", "・確定前に欠品を", "・再注文を1操作で"]]],
  "emotions": [1, 0, -2, 1] }
```
