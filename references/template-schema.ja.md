  ...
  "slides": [
    {
      ...
      "textFit": "shrink",
      // 図が作った図形をグループ化するか（既定: false）。true にすると、1 つの
      // figures 項目が作った図形 — flow の箱と矢印、カードの枠と見出しと本文 —
      // が 1 オブジェクトにまとまり、編集時にまとめて動かせる。見た目は変わらない。
      // 表とプレースホルダーは API がグループ化を拒否するため、画像は枠合わせを
      // 生成後に適用する都合で、いずれも対象外。slide 単位・"defaults"・図ごとに
      // 書ける（図の "group": false でその図だけ外せる）
      "group": true,
...
| `list` | Slides **ネイティブ**の箇条書き・番号付きリスト（`createParagraphBullets`）。文字として「・」を書くのではないので、Slides 上で編集しても PowerPoint に書き出しても記号が保たれる。`style` は `bullet`（既定）か `numbered`、`preset` で API のプリセット名を直接指定できる。項目は文字列、`{"text": …, "items": [...]}`（入れ子。3 階層まで）、`{"text": …, "level": n}` のいずれか。`size` / `lineSpacing` / `itemGapPt` / `indentIn` / `hangingIn` は任意。`indentIn` / `hangingIn` はプリセットのインデントを全階層で上書きするので、入れ子のときは指定しない。この型はテキストの自動適合を行わない（縮小のインデントがリストのインデントと衝突するため）。枠は文字量に合わせて取り、残りははみ出し検査で拾う |
...
## 4. スライドテンプレートの `template.json`（別のファイル）

§1 が説明しているのは**ブランドテンプレート** — デッキの複製元マスターである
`templates/<id>.json` のこと。**スライドテンプレート**は名前が同じだけの別ファイルで、
スキーマも別物である: `slide-templates/<パック>/<id>/template.json`。デッキ仕様が
`$template` で名指しできる、出来合いの 1 枚だ（README と
[slide-template-catalog.md](slide-template-catalog.ja.md) の冒頭を参照）。
構成は 2 つ:

| キー | 中身 |
|---|---|
| `slots` | そのページが取る入力 — `type` / `required` / `default` と制約。カタログの**入力**表はこれを出している |
| `slide` | §2 のデッキ仕様スライドそのもの。ただし呼び出し側が埋める値はマーカーのまま書く |

レンダリングまで決まらない値は、3 種類のマーカーで置いておく。`$slot` は `slide` の
中だけ、`$density` と `$t` はどちらの部にも書ける。

### `{"$slot": "<名前>"}` — 呼び出し側の入力

そのスロットの値に置き換わる。優先順は `data` で渡された値、次にスロット自身の
`default`。どちらも無ければエラー（`no value for slot: <名前>`）で、テンプレートが
宣言していないスロットを渡した場合も同様。`required` なスロットは `slide` のどこかから
参照されていなければならない — 必須なのにページが刷らない入力はテンプレート側の
バグなので、レンダリング時に弾く。

### `{"$density": {"print": …, "presentation": …}}` — 密度ごとの値

2 つのキーは両方必須で、`$density` と並ぶ兄弟キーは置けない。どこかでこのマーカーを
使うテンプレートは、有効な `defaultDensity` を宣言していなければならない。密度の解決は
入力検査より**先**に走るので、`$density` オブジェクトで書いた `maxLength` は比較の
時点では既にただの数値になっている。密度はスライド → spec → `defaultDensity` の順。

### `{"$t": "<キー>"}` — スライドが自分で刷る語

`slide-templates/i18n/<lang>.json` から引く。表の見出し、軸の両端、`出典:` のような
語のこと。`$t` と並ぶ兄弟キーは置けず、キーは文字列でなければならない。要求した言語に
無いキーは、空にするのではなく既定言語（`ja`）へフォールバックする。未訳が空欄ではなく
「日本語のまま出ている語」として見えるようにするためで、どちらの言語にも無いキーは
エラーになる。

言語はスライド → spec → `ja` の順。`data` に書いた内容は翻訳されない。訳されるのは
テンプレート自身が持っている語だけである。テンプレート外で描く図も
`Canvas._label()`（[diagrams.md](diagrams.ja.md)）から同じリソースを引くので、
1 枚の中で言語が食い違うことはない。

スロットの `default` 自体を `{"$t": …}` にしてもよい。入力の型検査より前に解決するので、
`"type": "string"` のスロットはちゃんと文字列を見る。

`decision-record` は 3 つとも使っている:
...
展開は検証・生成・`--into` のいずれよりも前に走るため、`--dry-run --strict` を使えば
入力の不足や超過を、何かが作られる前に捕まえられる。
