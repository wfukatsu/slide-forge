...
- `config/settings.json` が既に決めていること — Gemini で画像を生成するかどうか、
  成果物を Google Slides にするかローカルの `.pptx` にするか。**Q2 の前に読み**
  （`.venv/bin/python scripts/settings.py --show`）、設定が答えている質問は落とす。
  詳細は `references/settings.ja.md`。設定の**変更**を頼まれたら、ここではなく
  `settings` スキルに回す。
...
| 2 | 用途 | このデッキは何に使いますか？ | 提案書・営業資料（Proposal 系＋テンプレート密度 `print`）／ 登壇・勉強会（Presentation 系＋密度 `presentation`）／ 社内共有・報告（Proposal 系＋密度 `print`）。この回答が**レイアウト系統と、デッキで使う登録テンプレートの密度バリアントの両方**を決める。テンプレートが系統を持たない場合も省かず、縮約 2 択「このデッキはプレゼン投影用ですか、印刷・配布用ですか？」（投影用 → `presentation` ／ 印刷・配布用 → `print`）をテンプレートスライドを使う限り必ず問う |
| 3 | 構成 | どの構成で組みますか？ | [references/deck-outlines.md](deck-outlines.ja.md) の型から 3 つ＋「内容に合わせて組む」。**`preview` にセクション見出しを入れる**と比較しやすい |
...
**密度の回答の反映方法。** `$density` バリアントを持つ登録テンプレート
（例: `read-alone` パック）は、選ばれた密度をすべての
`render_slide_template.py --density …` 呼び出しに渡して描画する（バリアントの
ないテンプレートでは無視される）。`print` のデッキでは、テンプレートを使わない
本文スライドも配布密度に合うよう、仕様の `defaults`（例: `"bodyFontSize": 10`）
を併せて設定してよい（既存のスペック機能で、エンジン側の変更は不要）。
...
| 出力形式 | Google Slides に加えて PowerPoint (.pptx) も書き出しますか？ | Google Slides のみ（既定）／ PPTX も書き出す（納品・配布用。`pptx-export` スキルで生成後にエクスポート）。**`config/settings.json` の `output` が設定済みならこの質問はしない** — `local` は .pptx が成果物という意味 |
...
- **AI 画像（`aiImage`）は課金済みの `GEMINI_API_KEY` が要る**。使う前提の回答が
  ...
  429 で失敗する。**`config/settings.json` の `imageGeneration` が OFF なら、
  AI 画像を提案せずキーの有無も聞かない** — 図形で描く手段に直行する
  （どのみちエンジンが検証時に `aiImage` を弾く）。
...
- **「出力形式」は PPTX での納品・配布が想定されるとき（提案書・顧客向け資料など）
  で、かつ `config/settings.json` が未設定のときだけ聞く**。`output: local` なら
  生成後に自動でローカルフォルダへ書き出されるので、質問ではなく前提として
  一言伝え、結果報告でデッキ URL と並べてローカルパスも出す。
  登壇資料や社内共有では聞かずに Google Slides のみとする。
