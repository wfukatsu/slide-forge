...
[account-graphs.md](account-graphs.ja.md) を参照。
...
**大きい箱は角丸にしない。** Slides の角丸半径は固定長ではなく短辺の 1/6 なので、
箱が大きくなるほど丸みも大きくなり、パネルがぼてっと見える。`Canvas.shape()` は
短辺が `MAX_ROUND_SIDE`（0.60in）を超える `ROUND_RECTANGLE` 系を自動で `RECTANGLE`
に落とす（半径が 0.10in を超えない範囲）。チップや細い帯は短辺が小さいので角丸の
まま残る。呼び出し側で `kind` を変える必要はない。
