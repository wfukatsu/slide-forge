...
**詰めるのは文字の揃えと反対側の辺だけ。** START（左揃え）は右端、END（右揃え）は
左端、CENTER は両側を動かす。プレースホルダは左インデントが箇条書きを担うので、
常に右端だけ。左揃えのタイトルを両側で詰めると、他のページのタイトルより約 0.06in
左から始まってしまう（サムネイル QA で発見）。

**適合では列幅の 95% を使える幅として数える。** Slides は列幅ちょうどの行を折り
返す。実測では 2.6in・12pt の枠で式は全角 15.0 文字を示すが、入るのは 14 文字で
15 文字目は折り返す（`TEXT_BOX` / `RECTANGLE` / `ROUND_RECTANGLE`、Noto Sans JP /
Arial のいずれも同じ）。検査は従来の式のまま、適合だけが狭い列幅
（`_text.FIT_WIDTH`）で見積もるので、「収まる」と判断したものは実際に収まる。

**1 行の高さは フォントサイズ × 1.2 × lineSpacing で、1.45 ではない。** 8 行の
テキストボックスをサムネイルから読み取った実測では、lineSpacing 100 で 1.19、
115 で 1.38、120 で 1.44、150 で 1.81。Noto Sans JP でも Arial でも、12pt でも
20pt でも同じだった。文字の塊は枠の上端から約 0.05in 下がって始まる。以前の
1.45 は Noto Sans JP の ascent + descent で、どの塊も約 2 割高く見積もっていた
ため、適合が必要以上に文字を縮めていた。`_text.LINE_EM` は 1.2 にした。縮小では
行全体が枠に入れば十分とする（0.05in のずれは枠の下の内側余白に収まり、低い枠の
1 行ラベルは縮めない）。`grow` は広げる高さにこのずれ（`_text.FIT_INSET_Y`）を
足す。
