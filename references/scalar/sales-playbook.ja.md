...
ステージ内で「誰に会い、何を渡し、何を持ち帰るか」の粒度は
[stage-io-map.ja.md](stage-io-map.ja.md) が持つ。あちらはここを参照するが、
**ゲートと判定基準はここが唯一の出典**である。
...
| # | 原則 | 資料づくりへの含意 |
|---|---|---|
| 1 | 営業は確率論で管理する（接点数・勝率・サイクル・TCV） | 勝率と金額の低い商談に、正式提案の工数を割かない。**その数字は CRM で見る**（§8） |
...
- **テリトリー全体の数字（ステージ別勝率・平均サイクル・パイプライン充足率・
  ナーチャリングの段別移行率）も CRM / MA を正本とする。** 原則 1 の「確率論で管理」は
  ここで回す。**slide-forge 側に集計用の様式を作らない**（`templates/sales/` は商談
  1 件、`templates/nurture/` はセグメント 1 本の単位しか持たない）。
  個々の記録は集計の入力であって、集計結果の置き場ではない。
