...
`okf/pricing/` はこのバンドルで唯一、上流を持たないセクション。株式会社 Scalar の
価格表（ScalarDB / ScalarDL は 2024-07-01 版、ScalarDB Analytics は 2024-09-10 版）から
書き起こされている。**バンドルのリポジトリは公開されており、収録されている情報は
公開情報として参照してよい** — 定価を含む。境界は「社内 / 社外」ではなく、
**バンドルが公開したもの / 意図的に伏せたもの**である。

- **公開・引用可**: ScalarDB Analytics の SDBU・時間単価、Pod サブスクリプション 4 製品の
  月額・年額定価。加えて課金モデル、Pod 数の数え方、エディションの内容。
- **非公開・引用不可**: 3年契約の定価、先払いクレジット価格、値引き率、顧客固有の契約条件は
  すべて「非公開」と明記され、意図的に載っていない。提案で必要になったら「非公開」である旨を
  述べて営業担当につなぐ。月額・年額から内挿しない。
- 記載はすべて **定価・税抜・JPY**。定価であることを明示し、**参考見積の材料**として使う —
  確定金額・提出可能な見積としては出さない。実際の提供条件はボリュームと契約条件で調整される
  ため、顧客に出す見積は営業担当のレビューを通す。
