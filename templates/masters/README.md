...
環境に用意します。** 手順は README の「セットアップ > 5. スライドマスター」を
参照。

**他人のマスターへのアクセス権は前提ではありません。** どの登録済みマスターに
到達できるかは次で確認できます。
...
どれにもアクセスできない場合は、`template-forge` スキル
（`scripts/build_template.py`）で自分のブランドのマスターを新しく作り、登録
できます。以下の .pptx 経由の取り込みは、マスターを受け取っている場合の道です。
