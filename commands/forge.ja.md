...
「用途」の回答は**テンプレート密度**も決める: 提案書・社内共有 → `print`、
登壇 → `presentation`。`$density` バリアントを持つテンプレート（例:
`read-alone` パック）の `render_slide_template.py` 呼び出しにはこの密度を
`--density` で渡す。マスターが Proposal / Presentation 系統を持たない場合も
質問を省かず、縮約 2 択（投影用 / 印刷・配布用）で問う。
