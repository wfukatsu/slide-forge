...
- 指標の目標値を作らない。**まず 1〜2 四半期の実績を取ってから置く**方針にしてある
  （`nurture-map.ja.md` §7-10）。目標欄は「（実績待ち）」のままにする。
  ただし**測定元（MA / CRM のどの項目を見るか）は埋める** — 決めていないと実績すら取れない。
- メールの配信間隔には既定値がある（段 0〜1 は月 1 回、段 2 は隔週、段 3〜4 は週 1 回）。
  変えてよいが、**変えたらなぜ変えたかをトラックに書く**。**同一リードへの送信は月 4 回まで**。
