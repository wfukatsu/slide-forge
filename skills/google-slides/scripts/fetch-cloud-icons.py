...
3. **マニフェスト `cloud-icons.json` を作る。** 名前で引けるようになる。
   ...
   `02390-icon-service-azure-sql.png` のような名前で引けなかった。
    ...
    python scripts/fetch-cloud-icons.py                 # 3 ベンダーとも取り込む
    python scripts/fetch-cloud-icons.py --vendor azure  # 1 つだけ更新する
    python scripts/fetch-cloud-icons.py --dry-run       # 取得せず URL だけ確かめる

既定では google-slides / google-slides-template の両スキルへ同じものを配置する。
...
# 配置先。google-slides は assets/shared/、google-slides-template は assets/ の下
DESTS = [
    os.path.join(SKILL_DIR, "assets", "shared", "cloud-icons"),
    os.path.join(SKILLS_ROOT, "google-slides-template", "assets", "cloud-icons"),
]
