...
- 完全なソースSpecからslide-forge管理デッキの指定ページだけを更新する、または
  全ページ置換の明示承認後にURLを維持したまま再構築する
...
- 局所修正はsnapshot後に `--into <deck> --update-slides <pages>` を使い、未選択ページを
  保持する。`--update-slides` なしの `--into` は全ページを置換するため、対象確認と
  全ページ置換の明示承認を必須とする。
...
```
   ...
   問題がなければ 3 行の要約だけが出る。スライドごとのレイアウト一覧や、
   縮めたテキストの全件が要るときだけ `--verbose` を付ける。
