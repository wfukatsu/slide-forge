...
- **組み立てと生成。** `assemble-spec.py` → `--dry-run --strict` → `build-deck.py` は
...
.venv/bin/python scripts/assemble-spec.py --out /tmp/chk-$$.json --title chk \
...
```bash
.venv/bin/python scripts/fetch-thumbnails.py "<URL>" --out out/<deck>/qa
```
...
- **断片に `title` を書かせない。** デッキのタイトルは `assemble-spec.py --title` で
