...
`assets/icons/` に Scalar ブランドのピクトグラムが **62 種**入っている。24px グリッドの
...
ラスタライザがひとつも無い環境では `assets/icons/png/` の素材をそのまま使い、
...
1. SVG を `assets/icons/svg/<slug>.svg` に置く（24×24 の viewBox、単色 #C7C9C9）
2. `assets/icons/icons.json` に `ja` / `en` / `tags` / `recolorable` / `colors` を足す
3. 控えの PNG を焼く:
