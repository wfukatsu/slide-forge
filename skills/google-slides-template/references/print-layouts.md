...
| ファイル | 枚数 | 何のためのデッキか |
|---|---|---|
| `examples/print-deck-catalog.json` | 30 | **型の使い方を学ぶ。** 各部品の役割・良い例と悪い例・アンチパターン集 |
| `examples/print-pattern-index.json` | 56 | **作れるページを見て選ぶ。** 1 枚 = 1 パターンの実物索引 |

新規に印刷配布用デッキを作るときは、まず `print-pattern-index.json` を生成して
「どのページで組むか」をユーザーに指させ、選ばれたパターンで仕様を書くのが速い。

## ページのパターン = 骨格 × 中身

印刷配布用のページは 2 つの掛け算でできている。**骨格は 6 種しかない。**

| 骨格 | 構成 | 使いどころ |
|---|---|---|
| A 全幅 1 図 | タイトル + リードイン + 図（全幅） + 出典 | 表や大きなツリーなど、縮めると読めない図 |
...
中身（図表の枠に何を入れるか）は 7 分野 35 種で、`print-pattern-index.json` の
...
- **索引・一覧の表は 12 行前後が上限。** Slides のテーブル行は 0.28in 未満に
  縮まないので、`row_h` を詰めても収まらない。行数が多いなら分野で割って
  複数枚にする（`print-pattern-index.json` の索引が 4 枚あるのはこのため）。
