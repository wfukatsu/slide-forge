...
ような業務語彙は描けない。そこは `assets/icons/` のブランド素材（62 種）を使う。
...
| `fit` | 挙動 |
|---|---|
| `contain`（既定） | 比率を保って枠内に収める。余白ができる |
...
- `aspect` を省略すると枠の縦横比に最も近い比率が選ばれる。
...
1. Drive へ一時アップロードする
2. 「リンクを知る全員が閲覧可」を付ける — `createImage` は URL を**匿名で**
   取りに行くため、認証済みの自分がアクセスできるだけでは足りない
3. `createImage` で挿入する（Slides が画像をプレゼンテーション内へコピーする）
4. `commit()` の直後に一時ファイルを削除し、既存ファイルに付けた公開共有を外す
   （`AssetStore.cleanup()`）
...
`build-deck.py` の spec で、スライドに `figures` を足せる。
...
`build-deck.py` は spec から生成するとき、これを自動で回して結果を表示する。
