...
| `calendar-slides` | 日付つきのタスク・予定を**カレンダースライド**にし、`slide-templates/calendar/` パックとその描画プリミティブ（`month_calendar`、`day_gantt`、`day_agenda`、`week_timetable`、`sprint_calendar`、`year_calendar`、`deadline_countdown`）を保守する: 月間カレンダー（月曜/日曜始まり、1 週 1 行、複数日の帯、「+N件」への畳み込み）、日単位ガント（60 日までは 1 列 1 日、26 週までは 1 列 1 ISO 週に切り替え。土日祝の網掛け、今日線、営業日数）、日次タスクリスト（1 行 1 タスクで下方向、日付セルの結合、連続する休日の畳み込み、状態チップ）、週間タイムテーブル（曜日の列 × 時間の行、重なる予定は 2 件まで横並び、休憩の帯）、スプリントカレンダー（連続する 1 週/2 週スプリント、祝日を除いた稼働日数、計画・レビュー・リリース日の自動配置）、年間カレンダー（年度初めから 12 か月の小さな月、繁忙期・休業の面と重要日の丸）、期限カウントダウン（暦日と営業日の残り日数、節目 3 件まで、期限までを塗ったミニカレンダー）。祝日は同梱の内閣府 CSV `assets/holidays/jp.csv`（`scripts/update_holidays.py` で更新）、会社独自の休業日は `extraHolidays` で渡し、長い期間は `scripts/calendar_pages.py` がページに分割する。スキーマ・検証・登録は `slide-template-creator` のルールに従う。月単位の計画（`planning/gantt-schedule`）やマイルストーン年表（`planning/milestone-timeline`）には使わない。 |
...
slide-templates/ 再利用可能な 1 枚ものコンテンツテンプレート + レジストリ (14 パック 99 種、manifest.json)
...
[ガイド付きセットアップのプロンプト](#ガイド付きセットアップ--このプロンプトをコピーする)を使えば対話的に進められる。
...
### ガイド付きセットアップ — このプロンプトをコピーする

下の手順 1〜8 がリファレンス。案内されながら進めたい場合は、slide-forge の
ルートを作業ディレクトリにしてエージェント（Claude Code、Codex など）を
起動し、次のプロンプトを貼り付ける。何を使うつもりなのかを先に尋ね、その
回答から必要になる手順だけを実行し、ブラウザでの作業はこちらに返してくる。

```text
このマシンで slide-forge を初めてセットアップして。

作業は slide-forge のルートで行う — クローンしたディレクトリ、Claude
プラグインなら ${CLAUDE_PLUGIN_ROOT}。そこにある README.ja.md の
「セットアップ」節を読むこと。手順 1〜8 が正典なので、独自に判断せず
そのとおりに進める。README.ja.md が見つからない場合は、何かをする前に
パスを尋ねること。

まず AskUserQuestion で質問する — 4 問を 1 回の呼び出しにまとめ、推奨の
選択肢を先頭に置く。そのツールが無いホストなら、同じ 4 問を番号付きで
提示して回答を待つ:
...
4. CLI メッセージを英語と日本語のどちらにするか（GSLIDES_LANG）。

そのうえで、この順に進め、回答から不要になったものは飛ばす:
- 手順 1（Python 環境）: .venv を作り requirements.txt を入れる。.venv が
  既にあるなら作り直さず、動くことを確認する。
- 手順 2（OAuth クライアント）: これはこちらがブラウザでやる作業。Google
  Cloud Console を自動化しようとしないこと。config/credentials.json が
  既にあるかを確認し、無ければ README の番号付き手順をそのまま提示して、
  こちらがファイルを保存するまで待ってから次に進む。
- 手順 3（確認）: scripts/list_templates.py を実行し、同意画面はこちらに
...
- config/ 配下の中身 — 認証情報、トークン、API キー — は表示も echo も
  cat もしない。ファイルの有無だけを報告する。
- こちらのブラウザやアカウントが要ること — Google Cloud Console、同意
  画面、Gemini キー、マスターの Drive へのアップロード — はいったん止めて
  こちらに尋ねる。代わりにアカウントを作ったり認証情報を入力したりしない。
- 途中で 1 つ失敗しても終わらせない。独立して進む分は最後までやり、
  残っているものを述べる。

最後に短い表でまとめる — 各手順が 完了 / 省略（理由）/ こちら待ち、
動作確認のために叩けるコマンドを 1 つ、そして今使えるスキルと
まだ使えないスキル。
```
...
`slide-templates/` には 14 パック・99 種の 1 枚ものテンプレートがある。座標では
...
99 の既製 1 枚ものテンプレートが登録されている。それぞれレンダリング済み画像、
答える問い、ガードレールつきで
[`references/slide-template-catalog.md`](references/slide-template-catalog.ja.md)
